...
fig = ppsd.plot_temporal([1./10.], legend=False, grid=True, show=False)
...
plt.tight_layout() 
plt.savefig('plotPaper_SelFreq_Ndays_paper.png', dpi=300)
...
fnTC = 'Drakewell 1426 - 1 hour data - 23Apr22-02May22.csv' # R36CB Cheadle Hulme 
...
axs.plot(dtList, fLightCountList, label='Light vehicles', color='darkorange') 
axs2 = axs.twinx()
axs2.plot(dtList, fHeavyCountList, label='Heavy vehicles', color='darkorchid') 
#axs.plot(dtList, fTotalCountList, label='Total vehicles', color='C5') 
axs.grid(True)
axs.set_ylabel('Number of light vehicles')
axs2.set_ylabel('Number of heavy vehicles')
...
axs.set_ylim(0, max(fLightCountList)*1.05)
axs2.set_ylim(150, max(fHeavyCountList)*1.05)
plt.tight_layout() 
plt.savefig('plotPaper_Traffic_Ndays_paper.png', dpi=300)
